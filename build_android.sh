...
NDK_HOME=/opt/dev/android/sdk/ndk/17.2.4988734
PREFIX=out/android
...
build(){
    ...
    CONFIGURATION="$CONFIGURATION --cc=$TOOLCHAINS/bin/$TOOL_PREFIX-gcc"
    CONFIGURATION="$CONFIGURATION --cxx=$TOOLCHAINS/bin/$TOOL_PREFIX-g++"
    CONFIGURATION="$CONFIGURATION --strip=$TOOLCHAINS/bin/$TOOL_PREFIX-strip"
    ...
    echo -e "-------- > Start config makefile with \n$CONFIGURATION --extra-cflags="${EXTRA_CFLAGS}" --extra-ldflags="${EXTRA_LDFLAGS}""
    ./configure ${CONFIGURATION} \
        --extra-cflags="$EXTRA_CFLAGS" \
        --extra-ldflags="$EXTRA_LDFLAGS"

    if [ $? != 0 ]; then
      echo "configure failed with code: $?"
      exit 1
    fi
...
build_all(){
    ...
    # COMMON_OPTIONS="$COMMON_OPTIONS --enable-avresample" # 过时了，要用 libswresample 库替换
    ...
    #choose which want to build
    echo "build target $build_target"
    if [ "all" == "$build_target" ]; then
      build "armeabi"
      build "armeabi-v7a"
      build "arm64-v8a"
      #build "x86"
